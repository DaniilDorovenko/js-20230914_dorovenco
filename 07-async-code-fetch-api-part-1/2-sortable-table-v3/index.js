...
export default class SortableTable extends SortableTableV2 {
  ...
  async sortOnClient(id, order) {
    console.log('sorting on client')
    await this.loadData()
    this.sort(id, order)
  }

  async sortOnServer(id, order) {
    console.log('sorting on server')
  ...
  async sortOnClick(event)  {
    ...
    if (targetCell.dataset.sortable === "true") {
      ...
      this.isSortLocally ? await this.sortOnClient(field, order) : await this.sortOnServer(field, order);
    }
  }
